...
try {
    ...
    $statsStmt = $pdo->prepare("
        SELECT 
            ...
            COALESCE(SUM(CASE WHEN ss.submission_status = 'graded' THEN ss.total_score ELSE 0 END), 0) as total_points_earned,
            COALESCE(SUM(CASE WHEN ss.submission_status = 'graded' THEN ss.max_score ELSE 0 END), 0) as total_possible_points,
            COALESCE(AVG(CASE WHEN ss.submission_status = 'graded' AND ss.percentage IS NOT NULL THEN ss.percentage END), 0) as average_percentage
    ...
    // Get pending activities (not started or in progress)
    $pendingActivitiesStmt = $pdo->prepare("
        ...
        LIMIT 10
    ");
    $pendingActivitiesStmt->execute([$student_id]);
    $pendingActivities = $pendingActivitiesStmt->fetchAll(PDO::FETCH_ASSOC);

    // Get activity of the day (nearest deadline or most recent)
    $activityOfDayStmt = $pdo->prepare("
        ...
        LIMIT 1
    ");
    $activityOfDayStmt->execute([$student_id]);
    $activityOfDay = $activityOfDayStmt->fetch(PDO::FETCH_ASSOC);

    // Get subjects with active recitations count
    ...
    // Get class leaderboard (students in same grade level)
    $leaderboardStmt = $pdo->prepare("
        SELECT 
            ...
            COUNT(CASE WHEN ss.submission_status = 'graded' THEN 1 END) as completed_activities,
            COALESCE(AVG(CASE WHEN ss.submission_status = 'graded' AND ss.percentage IS NOT NULL THEN ss.percentage END), 0) as avg_percentage
        FROM users u
